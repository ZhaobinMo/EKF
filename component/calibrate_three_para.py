...
class Looper():
    ...
    def init_pos(self, loops):
        self.loop_locs = loops

    def _get_loop_data(self):
        st = 0  # index
        ed = self.L   # index

        loop_locs = self.loop_locs
        print('------location of loop: {} ------'.format(loop_locs))
        self.rho_loops = [self.rho_mat[i, :] for i in loop_locs]
        self.q_loops = [self.q_mat[i, :] for i in loop_locs]
    ...
    def _calibrate(self, rho, q):
        ...
        popt, pcov = optimize.curve_fit(self._Q, x, y, maxfev=1000000)
        return popt
